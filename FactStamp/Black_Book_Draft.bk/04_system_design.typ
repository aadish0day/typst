...
System design serves as the fundamental architectural bridge transforming the analytical specifications, sociotechnical requirements, and empirical constraints of *FactStamp* into an integrated, resilient, and horizontally scalable implementation blueprint. Addressing the viral proliferation of misinformation across closed WhatsApp networks in India requires an architecture capable of reconciling high viral velocity, multi-format media ingestion (unstructured text and low-resolution screenshots), encrypted peer-to-peer communication boundaries, and adversarial manipulation vectors. 

To achieve decentralized crowdsourced verification while maintaining rigorous institutional credibility, FactStamp adopts an edge-first, serverless architecture. This chapter presents an exhaustive decomposition of the platform's eight basic modules, the underlying real-time NoSQL data architecture, formal schema dictionaries, procedural and algorithmic execution pipelines, the Saffron Sleek user interface framework, comprehensive multi-layered security and Anti-Sybil defense models, and IEEE Std 829-2008 test case specifications.

== Basic Modules

The functional architecture of FactStamp is organized into eight specialized, loosely coupled, and cohesive basic modules. These subsystems delineate responsibilities across client-side multimodal intake, set-theoretic similarity deduplication, crowdsourced peer review, multi-parameter consensus mathematics, high-DPI visual synthesis, analytical intelligence, and holistic system defense.
...
- *Identity Management & Session Cryptography:* Built upon Firebase Authentication leveraging OpenID Connect (OIDC) protocols and Google Identity Services. Authenticated verifiers receive asymmetric RS256-signed JSON Web Tokens (JWT) containing cryptographic claims (`uid`, `email`, `email_verified`). Tokens undergo automated cryptographic refresh every 60 minutes over TLS 1.3.
- *Zero-Barrier Civic Intake:* Citizens submitting forwards or viewing debunked dossiers require zero authentication, ensuring frictionless accessibility. In contrast, evaluating claims, citing primary sources, and casting consensus votes strictly mandate authenticated verifier status.
...
- *Client-Side Canvas Downscaling & Storage Budget:* To eliminate recurring cloud storage fees, screenshots are downscaled in browser memory via an off-screen HTML5 `<canvas>` bounded to $1280 "px"$ maximum dimension. Quality steps down iteratively ($0.72$ down to $0.40$ in $0.08$ decrements) until the base64 payload is strictly below $700 "KB"$, permitting direct persistence on the Firestore claim document without breaching Firestore's $1 "MiB"$ limit.
...
- *Consensus Mathematics:* To resist Sybil attacks and sockpuppet brigading, the engine computes confidence $C in [0, 100]$ through three weighted parameters:
  ...
  Where $A$ is the agreement ratio ($%$) concurring with majority verdict, $R$ is average verifier reputation ($0-100$), and $S$ is average source domain authority ($30-100$).
...
- *Browser-Native SVG `<foreignObject>` Rasterization:* Rather than using legacy canvas parsers that fail when encountering modern CSS Color Module Level 4 tokens (`oklch()`) and Tailwind CSS v4 variables, Module 6 utilizes `html-to-image` to serialize live DOM nodes into an SVG container, drawing directly onto an off-screen HTML5 canvas at native browser execution speeds.
- *WhatsApp Form Factor:* Compiles an exact $1080 times 1080 "px"$ high-DPI PNG card with a square $1:1$ ratio fitting WhatsApp preview cards without center cropping. Features a Deep Saffron banner, dossier ID, quote-boxed claim text, tilted rubber stamp ($approx 6 degree$), JetBrains Mono confidence meter, and authoritative source tags.
...
- *Analytical Intelligence:* Highlights top debunked claims and maintains community leaderboards based on verification count and consensus alignment rate, rendered using Recharts and Framer Motion animated counters.
...
Data design represents a foundational engineering pillar of FactStamp. Unlike conventional web applications built upon rigid relational database schemas, FactStamp leverages a real-time, document-oriented NoSQL model powered by Google Cloud Firestore. This design satisfies the high-throughput, low-latency requirements of viral misinformation triage while accommodating semi-structured forward data, dynamic embedded verification arrays, and reactive client subscriptions.
...
#styled-table(
  ...
  "Data Structure Flexibility", "Rigid tabular schemas; DDL migrations require table locking.", "Flexible BSON/JSON document trees; dynamic schema evolution.", "WhatsApp forward submissions vary from short plain text to rich multimodal attachments with dynamic verifier lists.",
  ...
  "Operational Cost & Sustainability", "Continuous compute billing for idle server instances ($15–$50/month).", "Serverless consumption model with generous free Spark tier ($0/month).", "Preserves a 100% zero-cost infrastructure for civic longevity and university research sustainability."
)
...
Path: `/databases/{database}/documents/claims/{claimId}` The core transactional collection storing citizen forward submissions, image payloads, embedded verification arrays, and consensus outcomes.
...
Path: `/databases/{database}/documents/reports/{reportId}` Facilitates community moderation, allowing users to report abusive claims, toxic explanations, or suspicious sockpuppet activities.
...
This leaves a substantial $38%$ safety margin ($398.5 "KB"$), mathematically proving that FactStamp documents operate comfortably within Firestore's limits under extreme viral workloads without risking transaction aborts.
...
*Invariant C: Strict Privilege Separation on User Profiles* Users can modify their own display name, but cannot elevate their reputation score or grant themselves administrator privileges:
...
=== Comprehensive Empirical Step-Through Examples
...
5. *Threshold Evaluation:*
   $ 0.857 >= 0.75 ==> bold("DEFINITIVE DUPLICATE MATCH") $
6. *System Outcome:*
...
*Dual-Mode Contrast Calibration:* In dark mode, stark white fonts cause optical glare and haloing. FactStamp dynamically softens dark mode text to `oklch(0.92 0.010 55)` while elevating brand saffron to `oklch(0.72 0.18 48)`, maintaining $L_c >= 70$ across both display themes without color vibration.
...
- *Tabbed Modality Selector:* Citizens switch seamlessly between plaintext input (textarea bounded between $20$ and $2,000$ characters) and screenshot upload (drag-and-drop supporting JPEG, PNG, WebP up to $5$ MB).
...
Furthermore, low-quality source URLs submitted by Sybil accounts receive a source quality score of $S = 30$, mathematically capping confidence even under unanimous agreement ($A = 100\%$):

$ C_"attack" = round(0.40 times 100 + 0.30 times 50 + 0.30 times 30) = round(40 + 15 + 9) = bold(64%) < bold(70%) $

Because an authoritative Fact Card requires high certainty ($C >= 70%$), the system mathematically prevents unvetted sockpuppets from producing authoritative fact cards.
...
Test case design constitutes the formal verification and validation blueprint for FactStamp. In accordance with *IEEE Std 829-2008* (Standard for Software and System Test Documentation) and university dissertation guidelines, this section articulates the exhaustive test specifications formulated to validate all core functional, security, mathematical, and algorithmic subcomponents of the platform.
...
This comprehensive system design specification establishes the formal engineering blueprint, data invariants, and quality assurance framework governing FactStamp's project dissertation and production readiness.

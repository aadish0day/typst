...
Over the past decade, the rapid expansion of 4G and 5G telecommunications infrastructure, accompanied by hyper-affordable cellular data plans, has created the second-largest digitally connected population in the world, with over 850 million active internet users in India. Within this digital landscape, *WhatsApp* (Meta Platforms, Inc.) has transcended its original role as a casual peer-to-peer messaging application. With more than 535 million active users in India alone, WhatsApp operates as the de-facto civic, commercial, interpersonal, and informational backbone of the nation.

Everyday civic discourse, community alerts, local Resident Welfare Associations (RWAs), family networks, commercial trade groups, and political mobilization occur predominantly within WhatsApp group chats. However, the structural design principles that make WhatsApp extraordinarily compelling—namely *end-to-end encryption (E2EE)* via the Signal Protocol, automated address-book synchronization, low-friction one-tap message forwarding, and ubiquitous mobile accessibility—have simultaneously constructed the world's most potent vector for the unchecked dissemination of digital misinformation.
...
#block(
  ...
  [
    *Systemic Misinformation Propagation Failures:* \
    #v(3pt)
    1. *Dark Social Blindspot:* Closed encrypted chats conceal hoaxes until irreversible harm occurs. \
    2. *Asymmetric Diffusion Velocity:* Emotional falsehoods spread 6#text[×] faster than truth (2h vs 48h). \
    3. *Cognitive & Social Friction:* Sharing long, dry web URLs triggers interpersonal hostility. \
...
- *Hypothesis 2 (Duplicate Suppression):* A word-level tokenized Jaccard similarity index with stop-particle filtering ($|w| > 3$) and a calibrated threshold of $J >= 0.75$ can successfully identify $>= 95\%$ of viral WhatsApp forward variants (accounting for emoji alterations, greeting boilerplate, and minor typos) while maintaining a $0.0\%$ false-positive collision rate, redirecting users directly to canonical dossiers without redundant database writes.
...
#styled-table(
  ...
  "REQ-1", "Multimodal Claim Ingestion", "High", "Plaintext (10–2,000 chars) or Image (<=5 MB, JPEG/PNG/WebP/GIF); 5 categories; zero account mandate.",
...
- *REQ-7: 1080×1080px Fact Card Generator Subsystem*
  ...
  - *Processing & Business Logic:* Renders the card preview component utilizing the *Saffron Sleek* design system. Invokes `html-to-image` to clone the target DOM node, serialize it inside an SVG `<foreignObject>`, and rasterize it onto an HTML5 canvas at `pixelRatio: 2`. Encodes the canvas into a high-DPI $1080 times 1080$px PNG image blob.
...
- *REQ-10: Authentication Hardening & Session Security*
  ...
  - *Processing & Business Logic:*
    - *Login Rate Limiting:* `checkLoginRateLimit()` enforces a maximum of 5 failed login attempts (`MAX_LOGIN_ATTEMPTS = 5`) followed by a 15-minute lockout (`LOCKOUT_DURATION_MS = 15 * 60 * 1000`) with MM:SS countdown display, utilizing dual `localStorage` with `sessionStorage` fallback.
...
#styled-table(
  ...
  "NFR-4", "Usability & Accessibility", [UI compliant with *Saffron Sleek* design system; contrast complies with Accessible Perceptual Contrast Algorithm (APCA) ($L^c >= 75$ for body text); all interactive mobile touch targets measure >= 48×48px; responsive from 320px smartphones to 4K displays.],
  "NFR-5", "Reliability & Availability", [Target 99.9% platform availability backed by Vercel Edge Network and Google Cloud Firestore multi-region clusters; local-first offline caching via IndexedDB ensures UI operational resilience during brief network disconnects.],
  "NFR-6", "Zero-Cost Sustainability", [Entire system architecture must operate 100% within perpetual free-tier cloud quotas (Firebase Spark tier and Vercel Hobby tier), incurring exactly \$0.00/month in operational compute or database expenses.]
)
...
2. *Software Interfaces:*
   ...
   - *Tesseract.js (v5.x):* In-browser WebAssembly OCR worker utilizing pre-compiled language models executing in dedicated Web Workers.
...
#styled-table(
  ...
  "Agile Scrum", "Timeboxed 2-week sprints; rapid iterative releases; user demos.", "SELECTED MODEL: Perfect balance of empirical feedback, tuning, and agility."
)

#v(6pt)

*Why Agile Scrum Was Selected:*
1. *Iterative Problem Discovery:* In Sprint 3, when legacy canvas parsers failed on Tailwind v4's OKLCH color tokens, the team quickly pivoted within the active sprint to migrate to `html-to-image` without invalidating the entire project schedule.
...
The complete Activity-on-Node PERT network diagram highlighting the critical path ($S = 0$) and parallel streams is presented below:
...
#styled-table(
  ...
  "Developer Workstation", "Intel Core i5 / AMD Ryzen 5 (4 Cores), 8 GB DDR4 RAM, 256 GB NVMe SSD, 1080p Display.", "Intel Core i7 / AMD Ryzen 7 (8 Cores/16 Threads), 16–32 GB RAM, 512 GB–1 TB NVMe PCIe 4.0 SSD, Dual 1080p Displays.",
  "Target Mobile Device (Smartphone)", "Quad-Core ARM CPU (1.4 GHz), 2 GB LPDDR3 RAM, 3G / 4G LTE Cellular Network, 720×1280 Touchscreen Display.", "Octa-Core ARM CPU (2.0 GHz+), 4–8 GB LPDDR4X/LPDDR5 RAM, 4G+ / 5G / High-Speed Wi-Fi, 1080×2400 AMOLED 90/120Hz Display.",
  "Target Desktop Client", "Dual-Core x86_64 CPU (2.0 GHz), 4 GB RAM, Broadband Internet (2 Mbps), 1366×768 Screen Resolution.", "Quad-Core x86_64 CPU (2.8 GHz+), 8–16 GB RAM, High-Speed Fiber (25+ Mbps), 1920×1080 Full HD IPS Display."
)
...
- *Firestore Document Reads:* Because FactStamp leverages Firestore's local-first IndexedDB persistence, redundant queries across navigating sessions hit client cache rather than the cloud database. Even during a 10#text[×] traffic spike (12,000 reads/day), the system consumes less than 25% of the free allocation.
...
- *Platform Needs:*
  ...
  - _One-Click WhatsApp Export:_ Wants to download a clean visual card to post directly into his residential group to immediately halt rumor propagation.
...
- *Platform Needs:*
  ...
  - _Reputation Gamification:_ Transparent reputation tracking ($0–100$) and leaderboard recognition for consensus-aligned verdicts (+2 reward).
...
2. *Browser Sandbox & Mobile Memory Budget:*
   Budget mobile devices commonly encountered across India feature limited RAM (2 GB to 4 GB). FactStamp enforces strict client controls: canvas dimensions are clamped to $<= 1280 times 1280$px; dynamic JPEG quality stepping ($0.72$ down to $0.40$) guarantees base64 payloads under 700 KB (string size $<= 800,000$ chars); object URLs are promptly revoked (`URL.revokeObjectURL`) to release browser heap memory immediately.
3. *WhatsApp Square 1:1 Aspect Ratio Mandate:*
   WhatsApp's chat interface renders media previews by automatically cropping non-square images into centered square tiles in chat threads. Standard 16:9 or 4:3 cards suffer critical information loss: the top verdict badge and bottom source links are truncated. FactStamp mandates a strict *1:1 square aspect ratio* ($1080 times 1080$px), guaranteeing that the rubber-stamp verdict, claim summary, confidence meter, and sovereign citation URLs remain 100% visible in chat feeds without requiring users to expand the image.
...
- *Network Connectivity:* Assumes users operate on variable mobile networks (2G/3G/4G/5G). Pages utilize progressive enhancement and optimistic UI updates, functioning smoothly over intermittent connectivity.
...
#styled-table(
  ...
  "Claim Ingestion", "Anonymous plaintext forward submission (10–2,000 chars); screenshot upload with client canvas downscale (< 700 KB) and automated Tesseract.js WASM OCR extraction.", "Automated crawling of private WhatsApp chats; interception of WhatsApp network traffic; phone number scraping.",
  ...
  "Quorum Verification", "Public queue; minimum quorum threshold $N >= 3$; authoritative citation mandate; explanation validation (50–1500 chars); verifier reputation scoring ($0-100$).", "Single-moderator unilateral censorship; unweighted democratic popular voting; anonymous verifier voting.",
...
#styled-table(
  ...
  "Public Submitter", "Submit Claim", "Inflow -> System", "Plaintext message string (10-2,000 chars) or screenshot image file (<= 5 MB).",
...
#styled-table(
  ...
  "Verification Vote", "Community Verifier", "Process 3.0", "{ claimId, verdict, sourceUrl, sourceQuality, explanation (50-1500 chars), verifierId, verifierReputation }.",
...
#styled-table(
  ...
  "Main Success Flow", [1. Engine updates verifier civic reputation ($R$) bounded in $[0, 100]$. 2. Awards $+2$ for consensus alignment; applies $-1$ penalty for dissent. 3. Bounded reputation ensures Sybil accounts cannot elevate score without genuine consensus participation.],
...
3. *VERIFICATIONS (`embedded in /claims/{claimId}.verifications`):* Represents individual peer reviews. Stores `id`, `claimId`, `verdict`, `sourceUrl`, `sourceQuality` (`'high' | 'medium' | 'low'`), `explanation` (50–3,000 characters), `verifierId`, `verifierName`, `verifierReputation`, and `createdAt`.
...
2. *Pure Function Strategy (Algorithmic Determinism):* The consensus calculation (`confidenceScore.ts`) and duplicate detection (`duplicateDetection.ts`) algorithms are implemented as pure, side-effect-free TypeScript functions. This ensures 100% unit-testability without mocking external databases.
...
While a UML Class Diagram models classes, attributes, operations, and relationships at design-time, a *UML Object Diagram* captures a concrete snapshot of instanced objects in computer memory at a specific point in time ($t = t_"snapshot"$). This validates class model correctness, illustrates concrete collaborations, and verifies algorithmic state transitions under real operational conditions.
...
#styled-table(
  ...
  "EVT-01: Submit Plaintext Forward", "User pastes WhatsApp text and clicks 'Verify'", "Public Submitter (Browser)", "Strips null bytes and XSS tags, trims whitespace, validates length (10–2,000 chars), tokenizes words (|w| > 3).", "Displays processing indicator; initiates duplicate evaluation.", "Submit Page UI",
...
4. *Concurrent Peer Review Loop:*
   ...
   - Verifiers select a verdict (`TRUE`, `FALSE`, `MISLEADING`, `UNVERIFIABLE`), attach a primary HTTPS URL, and submit an explanation rationale (50–1,500 characters, $>= 8$ words).
...
- *Trusted Senior Verifier ($R > 80$):* High-accuracy track record. Highest mathematical consensus influence.
...
- *Stable Abstractions Principle (SAP):* Core domain modules (such as `confidenceScore.ts` and `duplicateDetection.ts`) possess zero external framework dependencies, ensuring 100% testability with standard unit test runners.
...
3. *Regional Proximity:* Cloud database clusters are provisioned within the Mumbai region (`asia-south1`), maintaining round-trip database latencies under 35 milliseconds for Indian end-users.
...
Component-Based Software Engineering emphasizes the design of software systems as assemblies of autonomous, loosely coupled, and independently replaceable modules. FactStamp is structured into nine specialized functional components connected through standard ball-and-socket interface conventions:
...
export interface IConsensusEval {
  calculateConfidenceScore(
    verifications: Array<{
      verdict: string;
      verifierReputation: number;
      sourceQuality: number; // 0–100 scale (100, 70, 30)
    }>
  ): {
    score: number;
...
- *Fault Isolation & Error Boundaries:* If client-side canvas memory allocation fails on ultra-low-memory mobile devices, React `ErrorBoundary` components isolate failures, providing graceful fallback notices without crashing the application process.
...
*Conclusion & Conceptual Architecture Verification:* \
The conceptual models presented across Chapter 3 establish an exhaustive, rigorous, and unambiguous blueprint of the FactStamp software architecture. By synthesizing structured data flow models, formal use cases, relational schemas, object-oriented class hierarchies, runtime memory snapshots, discrete event tables, activity workflows, finite state machines, chronological sequence traces, modular package structures, physical cloud deployment topologies, and component interface contracts, the system guarantees high architectural integrity, mathematical consensus validity, impenetrable security boundaries, and zero-cost serverless viability.

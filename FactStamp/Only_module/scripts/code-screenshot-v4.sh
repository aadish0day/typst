...
if [ $# -lt 2 ] || [ -z "$1" ] || [ -z "$2" ]; then
  echo "Usage: bash $0 <source-file> <output.png> [title]" >&2
  exit 1
fi
if [ ! -f "$CODE_FILE" ]; then
...
NUM_LINES=$(wc -l < "$CODE_FILE")
# 9px font, 1.4 line-height → 12.6px/line; header + pre padding + wrapper margins ≈ 62px
HEIGHT=$((NUM_LINES * 13 + 62))
...
python3 - "$HTML_FILE" "$B64_TITLE" "$B64_CODE" "$LANG_NAME" <<'PYEOF'
import base64, html, sys
html_file, b64_title, b64_code, lang = sys.argv[1:5]
with open(html_file, 'r') as f:
    content = f.read()
title = html.escape(base64.b64decode(b64_title).decode('utf-8', 'replace'), quote=True)
content = content.replace('__TITLE__', title).replace('__LANG__', lang)
...
if command -v firefox >/dev/null 2>&1; then
  ...
  firefox --headless --no-remote --profile "$TMP_FF_PROF" --screenshot="$OUTPUT_FILE" --window-size=700,${HEIGHT} "file://${HTML_FILE}" 2>/dev/null
  rm -rf "$TMP_FF_PROF"
elif command -v chromium >/dev/null 2>&1; then
  chromium --headless --disable-gpu --no-sandbox --disable-software-rasterizer \
    --screenshot="$OUTPUT_FILE" --window-size=700,${HEIGHT} \
    --hide-scrollbars \
...
echo "Written: $(stat -c%s "$OUTPUT_FILE") bytes to $OUTPUT_FILE (viewport 700x${HEIGHT})"

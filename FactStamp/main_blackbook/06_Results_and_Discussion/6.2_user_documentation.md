...
> Role-based user manual for FactStamp, screen by screen, matching the routes actually defined in `src/App.tsx`. Real screenshots are captured separately and are not fabricated here — every screen below carries a `[SCREENSHOT: <page name> — pending]` placeholder marking where the captured image will be inserted once available. Organized in two parts: **For End Users** (public/community routes) and **For Administrators** (the staff-only `/admin` console's 5 tabs).
...
A signed-in verifier's personal reputation page: an animated counter showing their current reputation score, a sparkline chart of that score's history, their current tier — Novice Verifier (0–30), Trusted Analyst (31–60), Expert Fact-Checker (61–85), or Elite Guardian (86–100) — a progress indicator toward the next tier, and a history of the verdicts they have personally submitted.

Each tier is labelled in the UI with a nominal consensus vote-weight perk: Novice Verifier and Trusted Analyst both read "Standard 1.0×", Expert Fact-Checker reads "Elevated 1.25×", and Elite Guardian reads "Maximal 1.5×" — so the first two tiers are in fact mechanically identical. This vote weight is presentational only. The label is produced by `repLevel()` in `src/pages/Profile.tsx` and is consumed nowhere else in the codebase: the consensus engine applies no tier multiplier, because `calculateConfidenceScore()` takes the *arithmetic mean* of the participating verifiers' reputations as its 30% component rather than weighting any individual verifier's vote by tier. Reputation therefore does influence a claim's confidence score, but through that averaged term, not through the per-tier multiplier the UI copy implies.

...
We evaluated FactStamp across three core dimensions: system throughput under concurrency, duplicate detection accuracy, and client-side rendering latency. Because the platform targets WhatsApp forwards circulating in Indian mobile messaging environments, benchmarking accounts for entry-level smartphone processors, throttled 3G and 4G networks, and degraded re-compressed screenshot artifacts.

==== Experimental Hardware and Client Environment Matrix
The test suite ran across four standardized hardware profiles representing common client devices, from entry-level budget smartphones to desktop workstations:
...
Across 1,200 evaluation runs spanning duplicate submissions with synthetic and natural linguistic permutations, the engine recorded the following performance metrics:
...
To determine why $J >= 0.75$ is the optimal decision boundary, empirical sweeps tested threshold values $J in [0.50, 0.95]$ in 0.05 increments. We measured the trade-off between True Positive Rate (Recall) and False Positive Rate (False Collisions) across 200 known forward variants and 300 distinct news claims:
...
The client-side compression pipeline enforces an upper boundary of $1280 "px"$ on the longest image dimension. The canvas downscales images via GPU bilinear interpolation and steps the JPEG quality factor ($Q = 0.72 -> 0.40$) until the payload fits below a $700 "KB"$ ceiling:
...
Optical character recognition runs floating-point matrix operations for LSTM language models. Running OCR synchronously on the main UI thread freezes the browser, dropping animation frames and delaying touch events. FactStamp offloads all OCR workloads to background Web Workers (`worker.min.js`):
...
- *UI Responsiveness:* Throughout active OCR parsing, the browser main thread maintained a steady $60 "fps"$ refresh rate with $0.0 "ms"$ of main-thread blocking latency.

=== Fact-Check Card Export Performance (`html-to-image`)

FactStamp produces a shareable $1080 times 1080 "px"$ square PNG card (`src/components/FactCheckCard.tsx`) using `html-to-image` with browser-native SVG `<foreignObject>` canvas rasterization.

==== Rendering Latency Benchmarks
Export latency must stay below 650 ms across target hardware profiles to support rapid sharing in active group chats:
...
All measured export latencies remained below the 650 ms target.

==== Architectural Comparison: `html-to-image` vs. Legacy JS Canvas Parser
During initial prototyping, a JavaScript-based canvas rendering library was evaluated. It failed on modern CSS color spaces and custom typography:
...
To keep FactStamp readable across varied lighting conditions and for users with low vision (including reading under direct sunlight), the interface applies the *Accessible Perceptual Contrast Algorithm (APCA)* developed for W3C WCAG 3.0.

Unlike legacy WCAG 2.1 flat mathematical ratios (which evaluate dark-on-light and light-on-dark contrast identically despite human ocular non-linearities), APCA calculates lightness contrast ($L^c$) based on spatial frequency, ambient adaptation, and display gamma:
...
Single-page applications that support light and dark themes often encounter a *Flash of Unstyled Content (FOUC)*: dark-mode users observe a brief white screen during initial HTML parsing before React state initializes.

FactStamp prevents this flash with an inline synchronous `<script>` inside the `<head>` of `index.html`. The script checks `localStorage` and system media queries (`prefers-color-scheme`), then writes the `data-theme` attribute and `.dark` class to `<html>` before the browser executes its first layout pass:
...
To confirm modular boundaries and verify the absence of circular imports, we evaluated the codebase with *Graphify* through automated Abstract Syntax Tree (AST) topological analysis.

The parser analyzed the production tree (Git commit baseline `4d7a654a`) and mapped all components, hooks, service providers, and configuration files into a structural knowledge graph:
...
Degree centrality analysis identified the most heavily connected modules in FactStamp, demonstrating that utility primitives and authentication hooks carry the highest fan-in:
...
To evaluate usability, turnaround time, and verification accuracy under realistic operating conditions, we conducted a 7-day peer trial with $N = 25$ student participants at *Jai Hind College (Empowered Autonomous), Mumbai*:
...
The production architecture supports daily active user traffic within the constraints of *zero cloud infrastructure expenditures*.
...
FactStamp supports three operational personas with varying levels of technical literacy across mobile messaging environments in India:
1. *The Submitter (Everyday WhatsApp User):* Individuals who receive suspicious forwards in WhatsApp groups and seek instant veracity checks without registration hurdles.
2. *The Verifier (Community Researcher / Student Fact-Checker):* Authenticated community members who utilize the verification queue workbench to investigate unverified claims, cross-reference primary and secondary evidence, submit structured verdicts, and build community reputation.
3. *The Administrator (System Moderator / Faculty Supervisor):* Authorized operators responsible for platform health oversight, review escalation, dispute resolution for contested claims, and anti-abuse enforcement.

The following sections detail the operational workflow for each persona, covering navigation steps, validation rules, and interface feedback states.

=== Submitter User Guide

The Submitter workflow allows a user to paste an unverified claim, evaluate duplicates, and retrieve a verified counter-card without intermediate registration hurdles.
...
5. *Review Extracted Text:* The OCR-extracted text populates an editable preview field where the user can correct misrecognized characters before submission.
...
The *TrustRing* displays the weighted algorithmic score ($C in [0, 100]\%$) computed from verifier agreement ($40\%$), average reputation ($30\%$), and source quality ($30\%$).
...
2. *Baseline Reputation Score:* Every newly registered verifier initializes with a neutral reputation score of *50 points* (scale 0 to 100) and zero recorded verifications.
...
- *Dynamic Replenishment:* The queue replenishes active claims from validated seed pools so verification tasks remain available.
...
- *Console Theme Controls:* The admin console provides a dual-icon theme toggle (`<ThemeToggle />`) in the header and a preference card in the Tools tab, switching between light and dark display modes without page reloads.
...
- *Administrative Action Options:*
  1. *Priority Flagging:* Flag claims for expedited review (`flagClaim`) to place them at the top of the community queue.
  ...
  3. *Verification Pruning:* Remove spam or collusive reviews with `deleteVerification` and automatically recompute claim consensus metrics.

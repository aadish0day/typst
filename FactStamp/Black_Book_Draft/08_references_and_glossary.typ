...
This glossary provides alphabetical definitions for 44 technical terms, algorithms, standards, and architectural concepts used throughout the *FactStamp* dissertation and software implementation. Each entry details the computing definition alongside its operational context within the platform.
...
#styled-table(
  ...
  [*Accessible Perceptual Contrast Algorithm (APCA)*], [A perception-based contrast prediction algorithm developed for W3C Web Content Accessibility Guidelines (WCAG) 3.0. Unlike legacy WCAG 2.1 flat mathematical ratios, APCA computes lightness contrast ($L_c$) from spatial frequency, ocular adaptation, font weight, and non-linear visual perception. \
  _FactStamp Context:_ Used to calibrate the OKLCH theme palette so body text achieves $|L_c| >= 75$ and interactive controls achieve $|L_c| >= 60$ in both light and dark display modes under high ambient illumination.],
  ...
  [*Agreement Ratio ($A$)*], [A normalized quantitative metric ($0 "to" 100\%$) indicating the degree of consensus among independent verifiers participating in a quorum. Computed as the count of votes supporting the majority verdict candidate divided by total submitted verifications: $A = (N_("majority") / N_("total")) times 100$. \
  _FactStamp Context:_ Accounts for 40% of the multi-factor confidence scoring formula ($C = 0.40A + 0.30R + 0.30S$).],
  ...
  _FactStamp Context:_ FactStamp applies BFT principles to its 3-verifier quorum model so consensus resolves correctly despite bad-faith outlier votes or burner accounts.],
  ...
  [*ClaimStatus*], [The formal lifecycle enumeration of a claim record in the FactStamp database (`src/lib/types.ts`), with two possible states: `'pending'` (active in the verification queue) or `'verified'` (resolved by quorum consensus or deliberation timeout). \
  ...
  _FactStamp Context:_ Primary cloud datastore for FactStamp's `claims`, `verifications`, `users`, and `notifications` within the free Spark tier.],

  [*Confidence Score ($C$)*], [A composite algorithmic rating ($0 "to" 100\%$) expressing the mathematical certainty of a certified fact-check verdict once a claim achieves the minimum 3-verifier quorum ($N >= 3$). \
  _FactStamp Context:_ Computed via the weighted linear combination $C = (A times 40%) + (R times 30%) + (S times 30%)$, which combines verifier agreement ratio ($A$), average historical reputation ($R$), and primary source credibility ($S$).],
  ...
  _FactStamp Context:_ Rendered with an electric blue badge (`hexColor: #2563eb`, `hexBg: #eff6ff`) to mark an unresolved controversy or evidentiary impasse.],
  ...
  _FactStamp Context:_ The operational environment targeted by FactStamp. Because WhatsApp misinformation spreads inside dark social groups, FactStamp enables users to extract claims and return visual counter-cards directly into the closed originating chat.],

  [*Data Flow Diagram (DFD)*], [A visual modeling notation representing the functional decomposition of a software system, showing the paths through which information moves from external entities through transformation processes to data repositories. \
  ...
  [*Devanagari Script Normalization*], [The computational linguistics process of normalizing Unicode codepoints for text written in Devanagari script (Hindi, Marathi), which standardizes matras (vowel signs), viramas, and conjuncts prior to set-theoretic tokenization. \
  ...
  _FactStamp Context:_ Implemented in `src/lib/duplicateDetection.ts` using set-theoretic tokenized Jaccard similarity; resolves duplicates with a mean latency of 78.4 ms at threshold $J >= 0.75$.],

  [*End-to-End Encryption (E2EE)*], [A secure communications mechanism where data is encrypted on the sender's device and decrypted only on the recipient's final device, preventing intermediate network switches, telecom providers, and server operators from reading message plaintext. \
  _FactStamp Context:_ Governs WhatsApp communications. FactStamp respects E2EE by functioning as an external, user-initiated verification tool rather than an automated chat scraper.],
  ...
  _FactStamp Context:_ Integrated in Milestone 2 (`graphify-out/`), which maps 89 source files, 1,743 nodes, 3,640 edges, and 94 architectural communities, identifying central modules such as `useAuth()`, `cn()`, and `firebaseService.ts`.],
  ...
  _FactStamp Context:_ Used in `src/pages/Submit.tsx` to downsample user-uploaded screenshots to a maximum 1280px boundary, yielding an 89.2% file size reduction before base64 encoding.],
  ...
  _FactStamp Context:_ Powers FactStamp's Fact-Check Card engine (`src/components/FactCheckCard.tsx`), generating 1080×1080px square PNGs in under 650 ms across tested devices.],
  ...
  _FactStamp Context:_ Powers the duplicate detection engine (`src/lib/duplicateDetection.ts`). Claims with normalized token overlap $J >= 0.75$ are classified as duplicates, with 96.4% suppression accuracy and 78.4 ms mean latency.],
  ...
  _FactStamp Context:_ Implemented in Milestone 5; enforces a 5-attempt threshold, a 15-minute lockout (`LOCKOUT_DURATION_MS = 15 * 60 * 1000`), account and client tracking in `localStorage` with `sessionStorage` fallback, MM:SS countdowns (`formatLockoutRemaining()`), and generic anti-enumeration errors.],
  ...
  _FactStamp Context:_ Implemented via Google Cloud Firestore, supporting hierarchical nesting of verification arrays within claim documents.],

  [*OKLCH Color Model*], [A modern cylindrical color space based on the Oklab appearance space, defined by Lightness ($L$), Chroma ($C$), and Hue angle ($H$). Unlike sRGB, OKLCH provides uniform perceptual lightness across different hues without visual brightness distortion. \
  ...
  _FactStamp Context:_ Handled client-side via Tesseract.js WebAssembly (`tesseract.js`), which extracts forward text from uploaded screenshots with 98.2% character accuracy on digital captures.],
  ...
  _FactStamp Context:_ Outlined in Chapter 7.3 as a planned extension using Service Workers and IndexedDB for low-bandwidth offline operation.],
  ...
  [*Reputation Score ($R$)*], [A numerical metric ($0 "to" 100$ scale) assigned to each authenticated verifier representing their historical accuracy record. New verifiers initialize at a baseline of 50. Accurate votes that match certified quorum consensus reward points ($+2$), while outlier votes deduct points ($-1$). \
  ...
  [*Source Credibility Score ($S$)*], [A normalized score ($0 "to" 100$) evaluating the trustworthiness of external evidence URLs cited by verifiers: \
  ...
  [*Tabular Figures (CSS tabular-nums)*], [An OpenType typographic feature (`font-variant-numeric: tabular-nums`) where all numerals share an identical advance width, avoiding visual jitter during counter and timer updates. \
  ...
  [*Tesseract.js*], [A JavaScript and WebAssembly port of the open-source Tesseract OCR engine that executes within browser Web Workers. \
  ...
  _FactStamp Context:_ Visualizes a verifier's reputation score on a 0 to 100 scale using color-coded progress strokes (green for high trust, saffron for good, amber for baseline, red for probationary).],
  ...
  [*Verdict Stamp*], [A visual badge displayed on verified claims and exported PNG cards. \
  _FactStamp Context:_ Renders one of five certified states: `TRUE`, `FALSE`, `MISLEADING`, `UNVERIFIABLE`, or `CONTESTED`, with distinct iconography and high APCA contrast.],
  ...
  _FactStamp Context:_ Resolves unverified claims older than 7 days into `CONTESTED` status via `applyLocalExpiry` while seed scripts generate claims with rotating future deadlines (3 to 6 days), so active claims remain in the queue during evaluation.],

  [*Vite 5*], [A frontend build tool and development server created by Evan You, with native ECMAScript Module (ESM) serving and Hot Module Replacement (HMR). \
  _FactStamp Context:_ Powers the FactStamp application build pipeline and development environment.],

  [*WebAssembly (WASM)*], [A binary instruction format for a stack-based virtual machine, designed as a portable compilation target for programming languages (C, C++, Rust) with near-native execution speed inside web browsers. \
  _FactStamp Context:_ Executes the Tesseract.js OCR engine and planned future Whisper and SLM inference pipelines directly within the client browser.],

  [*WebGPU*], [A web API providing hardware-accelerated 3D graphics and low-level parallel compute capabilities directly to web applications, succeeding WebGL. \
  ...
  [*Zero-Knowledge / Privacy-Preserving Architecture*], [A system design principle under which sensitive user communications, personal phone numbers, and private chat contexts are not logged, decrypted, or transmitted to centralized server backends. \

...
FactStamp sits between these two failure modes. It keeps the speed and reach of ad-hoc community debunking, but adds the source discipline and auditability of institutional fact-checking:

+ *A single shared record rather than per-group replies:* every claim is checked against a persistent corpus using Jaccard token-overlap similarity, so a resurfacing claim is sent to its existing verdict instead of being treated as new.
...
- Must operate entirely within Firebase Spark (free-tier) usage limits, with no paid backend infrastructure.
...
- *Algorithmic tuning risk* (Sprints 3 and 5): the Jaccard threshold and the 40/30/30 weighting were empirically tuned during development rather than fixed up front, which is why Agile/Scrum was chosen over Waterfall.
...
- *Solo-developer capacity risk*: sprint scope was deliberately kept to one or two modules per sprint rather than parallel workstreams, so each increment stayed realistically achievable for one person.
...
FactStamp's functionality is organized into 8 core modules, each responsible for one stage of a claim's lifecycle from initial submission through to a shareable verified verdict, plus a cross-cutting security layer and an admin moderation layer. The descriptions below are product-level; the data design, procedural logic, and algorithms behind each module appear in Chapter 4.

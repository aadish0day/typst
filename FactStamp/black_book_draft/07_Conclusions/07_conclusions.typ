...
+ *Eliminate redundant verification effort.* The Jaccard token-overlap duplicate detector (`src/lib/duplicateDetection.ts`, threshold of 0.75) recognizes a resubmitted claim even when its wording differs slightly, and returns the existing verdict instead of queueing the claim again.
...
+ *Weight consensus by evidence quality, not headcount.* The weighted confidence engine in `src/lib/confidenceScore.ts` synthesizes verifier agreement, participant reputation, and cited source credibility. Under it, a unanimous vote from low-reputation verifiers citing weak sources scores lower than one backed by trusted verifiers and authoritative citations.
+ *Make verified facts shareable in the native medium of the misinformation.* The `html-to-image`-based card generator (`src/components/FactCheckCard.tsx`) exports a 1080px-wide PNG, rasterized from a 540px-wide card at a pixel ratio of 2, with a height that grows with the claim text and source list. The card fits WhatsApp's dimensions and carries the verdict, confidence score, and cited sources back into the forward chains where the claim started.
...
=== Trust and governance limitations
...
- *Consolidating the quorum threshold into a single source of truth.* Replacing the hardcoded checks for at least 3 verifications in the client Context and service layer with one shared constant, or a Firestore remote-config value, would mean a future change to the quorum size takes one edit instead of six. Putting the value in `firestore.rules` as well would let the server enforce the quorum.

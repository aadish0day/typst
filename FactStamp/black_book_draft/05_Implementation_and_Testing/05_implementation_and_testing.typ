...
#styled-table(
  columns: (0.6in, 1fr, 1fr, 0.85in, 1.15in),
  headers: ("Case", "Claim A (incoming)", "Claim B (existing)", "Token Counts #sym.arrow.r |A|, |B|, |∩|, |∪|", "Expected Outcome"),
...
#styled-table(
  columns: (0.6in, 1.5fr, 1fr, 1fr, 0.7in),
...
#styled-table(
  ...
  "Login rate limiting #sym.amp lockout", "Security hardening", "No brute-force protection on sign-in / admin login", "`src/lib/security.ts`, `SignIn.tsx`, `AdminRoute.tsx`",
...
#styled-table(
  columns: (0.45in, 1fr, 1fr, 1.15fr, 1.15fr, 0.55in),
  ...
  "TC-09", "Verifier explanation --- too short", "Verdict with 4-char explanation (\"fake\")", "Rejected client-side before any Firestore write", "Rejected with expected error message", "Pass",
  ...
  "TC-13", "Consensus expiry to CONTESTED", "`consensusDeadline` passes with < 3 verifications", "Claim auto-settles `verdict: CONTESTED` via `applyLocalExpiry()`", "Claim correctly settled as CONTESTED at deadline", "Pass",
  ...
  "TC-15", "Fact-check PNG card export", "Export a resolved claim's card via `html-to-image`", "PNG at 1080px width (540px card @ `pixelRatio: 2`), height scaling to card content; OKLCH theme colors render correctly, no black regions", "Exported at 1080px width with correct colors; height varied with claim/source length as expected", "Pass",
  "TC-16", "Firestore rule rejects forged reputation write", "Direct write with inflated `verifierReputation` (bypassing UI)", "Write rejected by `firestore.rules`", "Write rejected with permission-denied error", "Pass",
  "TC-17", "Admin `sessionStorage` bypass attempt", "Manually set `fs_admin_session_unlocked` on non-admin account", "Bounced back to admin login gate; role re-derived from live snapshot", "Bounced back correctly; stale flag discarded", "Pass",
  ...
  "TC-22", "CI typecheck-and-build gate", "Push a commit to the repository", "`typecheck-and-build` job runs `tsc --noEmit` and `vite build` successfully", "Job completed successfully on the current codebase", "Pass",
)

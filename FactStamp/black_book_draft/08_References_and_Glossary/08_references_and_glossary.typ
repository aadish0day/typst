...
#styled-table(
  ...
  [Agreement Ratio ($A$)], [The percentage of verifiers on a claim who voted for the final majority verdict. This is the first term in the Confidence Score formula and has a 40% weight.],
  ...
  [Confidence Score ($C$)], [The final percentage from 0 to 100 generated by the Weighted Consensus engine. The formula is $C = 0.40 A + 0.30 R + 0.30 S$. It combines the Agreement Ratio, average Verifier Reputation, and Source Quality.],
  ...
  [Jaccard Similarity], [A similarity coefficient calculated as $J(A,B) = (|S_A inter S_B|) / (|S_A union S_B|)$. The system computes this between the normalized token sets of two claims. FactStamp flags a claim as a duplicate if $J >= 0.75$.],
  ...
  [Source Quality ($S$)], [A rating from 0 to 100 for the credibility of a cited source URL. The `determineSourceQuality()` function assigns 100 for high-quality government and health domains like `who.int` or `pib.gov.in`. It assigns 70 for medium-quality news sites like `bbc.com` and 30 for all other low-quality domains. The system averages these values to calculate the third term of the Confidence Score, which carries a 30% weight.],
  ...
  [Verifier Reputation], [A numeric score on a 0 to 100 scale, starting at 50, that tracks how often a verifier aligns with the community consensus. It determines the reputation term, $R$, in the Confidence Score.],

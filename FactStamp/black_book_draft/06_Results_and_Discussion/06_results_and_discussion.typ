...
#styled-table(
  ...
  "1", [Auth and Verifier Reputation], "8", "8", "0", "100%",
  "2", [Forward Submission and OCR Ingestion], "9", "8", "1", "89%",
  "3", "Duplicate Detection Engine (Jaccard)", "6", "6", "0", "100%",
  "4", [Verification Queue and Workbench], "7", "7", "0", "100%",
  "5", [Weighted Consensus and Confidence Engine], "6", "6", "0", "100%",
  "6", "Fact-Check Card Generator", "5", "5", "0", "100%",
  "7", "Misinformation Analytics Dashboard", "5", "5", "0", "100%",
  "8", [System Security and Notifications], "6", "5", "1", "83%",
...
// rebalanced (wider ID/Pass so "TC-01" and "Fail, then Fixed" stop spilling into
...
#styled-table(
  ...
  "TC-01", "New user registration", [Valid email and password via `SignUp.tsx`], "Auth account created; Firestore profile created with base reputation 50", "Account and profile created as expected; auto-signed-in", "Pass",
  ...
  "TC-04", "Text claim submission", [Plain-text WhatsApp forward pasted into `Submit.tsx`], [Claim created with `status` set to `pending`, added to Verification Queue], "Claim created and visible in queue within one write", "Pass",
  "TC-05", "Screenshot claim with OCR", "WhatsApp forward screenshot (JPEG) uploaded", "Image compressed; text extracted via Tesseract.js; WhatsApp chrome stripped", "Text extracted and chrome removed; shown for confirmation", "Pass",
  "TC-06", [Duplicate detection: identical claim], "Text identical to existing resolved claim", [Jaccard similarity of 1.00, at or above the 0.75 threshold; redirected to existing claim], "Redirected correctly; no new claim created", "Pass",
  "TC-07", [Duplicate detection: near-paraphrase], [Text differing by one word (5.3.1 Case 2, Jaccard similarity 0.80)], "Flagged duplicate; redirected", "Redirected correctly", "Pass",
  "TC-08", [Duplicate detection: related but distinct], [Topically similar, below threshold (5.3.1 Case 3, Jaccard similarity 0.56)], [*Not* flagged; new claim created independently], "New claim correctly created, not merged", "Pass",
  ...
  "TC-10", [Verifier explanation: valid], "Explanation of 50 characters or more with a real citation URL", "Accepted; appended to embedded array; count increases by exactly 1", "Accepted and appended correctly", "Pass",
  ...
  "TC-12", "3-verifier quorum consensus", "Three distinct verifiers submit a verdict each", [On 3rd verification: `status` becomes `verified`; score matches hand-computed value], "Transition occurred on 3rd verification; score matched exactly", "Pass",
  ...
  "TC-14", "Verification Queue empties after mass expiry", "All pending seed claims expire simultaneously", "Queue should replenish automatically, never empty", [Pre-fix: queue showed 0 claims (root-caused to 5.4 Mod. 1), then Fixed: auto-replenishes with dynamic-deadline seeds], [Fail, then Fixed],
  "TC-15", "Fact-check PNG card export", [Export a resolved claim's fact-check card], [PNG at 1080px width (540px card at `pixelRatio: 2`), height scaling to card content; OKLCH theme colors render correctly, no black regions], "Exported at 1080px width with correct colors; height varied with claim/source length as expected", "Pass",
  ...
  "TC-18", "Admin audit log completeness", "Claim-verdict override in Claims Moderation tab", [Corresponding `AdminAuditLog` entry written], [Audit entry created and visible in Audit and Tools tab], "Pass",
  "TC-19", "File-upload validation (disguised file)", [`.exe` renamed to `.jpg`, uploaded as screenshot], "Rejected by extension, MIME type, and magic-byte signature checks", "Rejected at magic-byte check despite renamed extension", "Pass",
  "TC-20", "Idle session timeout", "Authenticated session left inactive for more than 30 minutes", "Session auto-expires, requires re-authentication", "Session expired at the configured 30-minute threshold", "Pass",
...
The home page is public and needs no account. Its hero section plays an animated "Forward to Stamped Card" sequence in which a real example claim turns into a verdict stamp, alongside live platform statistics and two buttons, "Submit a Forward" and "Explore Verification Queue". Below the hero, a "Recently Debunked Claims" feed lists the latest resolved claims from Firestore, so visitors can see real verdicts before they register.

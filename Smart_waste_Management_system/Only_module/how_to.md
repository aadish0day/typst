# Smart Waste Management System Typst Submission Workflow

> Created by Buffy (AI agent) for Aadish — July 2026

## Overview

This document explains how Module 4 Typst submission was created: code screenshots from the Smart Waste Management System React project, embedded into a `.typ` document, and compiled to PDF.
...
1. **Short Code Snippets & Config Blocks (1–15 Lines):**
   ...
   - **Instead:** Use native Typst raw code blocks (```typescript ... ```) or the raw block styling handler defined in [`Typst_format.md`](file:///home/aadish/Documents/typst/Smart_waste_Management_system/Rules/Typst_format.md).
2. **Diagrams & Visual Schemas (UML, DFD, Architecture):**
   ...
   - **Instead:** Render vector diagrams via PlantUML (`.puml` → SVG) or Graphviz (`.dot` → SVG) as mandated in [`Diagram-rules.md`](file:///home/aadish/Documents/typst/Smart_waste_Management_system/Rules/Diagram-rules.md).
...
1. **Step 1: Check Content Type**
   ...
   * **UML Diagram** (Class, ER, State, Component, Use Case, Package): **Do NOT use `Only_module`** → Use PlantUML (`.puml` → SVG) per [`Diagram-rules.md`](file:///home/aadish/Documents/typst/Smart_waste_Management_system/Rules/Diagram-rules.md).
   * **DFD Diagram** (Level 0, Level 1, Level 2): **Do NOT use `Only_module`** → Use Graphviz (`.dot` → SVG) per [`Diagram-rules.md`](file:///home/aadish/Documents/typst/Smart_waste_Management_system/Rules/Diagram-rules.md).
   * **Short Snippet / Config / Shell Command (<15 lines)**: **Do NOT use `Only_module`** → Use native Typst raw block (`` ```ts ... ``` ``) per [`Typst_format.md`](file:///home/aadish/Documents/typst/Smart_waste_Management_system/Rules/Typst_format.md).
